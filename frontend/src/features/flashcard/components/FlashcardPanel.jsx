...
const FlashcardPanel = ({
    ...
    <div className="flex-1 overflow-auto p-5 md:p-6">
      {showGenerator ? (
        <FlashcardGenerator
          ...
          onCreateFlashcard={async (text) => {
            const createdCards = await onCreateFlashcard(text);
            if (createdCards) {
              setGeneratorPrompt('');
            }
          }}
        ...
        <FlashcardStudyView data={flashcards} isLoading={isLoading} error={error} />
      )}
    </div>
  );
  ...
  const expandedPanel = isExpanded && typeof document !== 'undefined'
    ? createPortal(
        <div
