...
import { useTheme } from "../../../components/theme/ThemeWrapper"; // <-- Import hook theme
          ...
          {files.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-[1.6fr_1fr] gap-8 animate-in slide-in-from-bottom-8 duration-500 h-[600px]">
              ...
              <div className="flex flex-col h-full overflow-hidden relative">
                <h3 className={`font-bold text-sm mb-3 shrink-0 ${isNight ? 'text-gray-300' : 'text-slate-700'}`}>Tệp đã chọn ({files.length})</h3>
                ...
                <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-3 pb-2">
                    ...
                    >
                      <div className="flex items-center gap-3 overflow-hidden">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className={`text-[10px] font-semibold mt-0.5 uppercase ${isNight ? 'text-gray-500' : 'text-slate-400'}`}>{(file.size / 1024 / 1024).toFixed(2)} MB</p>
                        </div>
                      </div>
                      <button 
                  ...
                  <button 
                    onClick={() => fileInputRef.current.click()} 
                    className={`w-full py-5 border-2 border-dashed rounded-2xl font-bold text-sm transition-all ${
                ...
                <div className={`mt-4 pt-4 border-t flex flex-col xl:flex-row justify-end gap-3 shrink-0 ${isNight ? 'border-gray-700' : 'border-slate-100'}`}>
                  <button onClick={() => setFiles([])} className={`rounded-full px-6 py-4 text-sm font-bold transition-colors ${
