        ...
        <div className="animate-train">
          {[...PATHS, ...PATHS].map((item, idx) => (
            <div key={idx} className="w-[12.5%] h-full px-1.5 pb-2">
              <div
                style={{ backgroundColor: isNight ? item.nightBg : item.bg, borderColor: isNight ? 'transparent' : item.border }}
              ...
              >
                <img src={item.icon} alt={item.title} className="w-14 h-14 shrink-0 object-contain drop-shadow-md group-hover:scale-110 transition-transform duration-300 origin-bottom" />

                <div className="flex-1 min-w-0 flex flex-col justify-center">
                  <div className="flex items-start gap-1 mb-0.5 min-w-0">
                    <h4 className="text-[13px] font-black tracking-tight leading-tight whitespace-normal break-words" style={{ color: isNight ? item.nightColor : item.color }}>{item.title}</h4>
                  ...
                  <p className={`text-[10.5px] font-bold leading-snug whitespace-normal break-words ${isNight ? "text-slate-400" : "text-slate-600"}`}>{item.desc}</p>
                </div>
              </div>
            </div>
          ))}
