...
const FormatMarkdown = ({ text, isNight }) => {
  ...
  return lines.map((line, index) => {
    ...
    if (cleanLine.startsWith('###') || cleanLine.startsWith('##')) {
      ...
      return (
        <h4 key={index} className={`font-black text-sm mt-4 mb-1.5 ${isNight ? "text-blue-400" : "text-blue-800"}`}>
          {headingText}
        </h4>
      );
    }
    if (cleanLine.startsWith('-') || cleanLine.startsWith('*')) {
      ...
      return (
        <li key={index} className={`list-disc ml-4 text-[12px] my-1 leading-relaxed ${isNight ? "text-slate-300" : "text-slate-600"}`}>
          {renderBoldText(bulletText, isNight)}
        </li>
      );
    }
    return (
      <p key={index} className={`text-[12px] my-1.5 leading-relaxed ${isNight ? "text-slate-300" : "text-slate-600"}`}>
...
const renderBoldText = (rawText, isNight) => {
  ...
  return parts.map((part, i) => 
    i % 2 === 1 
      ? <strong key={i} className={`font-extrabold ${isNight ? "text-white" : "text-slate-900"}`}>{part}</strong> 
      : part
  );
};
...
const EvaluationWindow = ({ 
  ...
  return (
    <div className={`w-[360px] h-[500px] rounded-[2rem] border-2 flex flex-col overflow-hidden transition-all duration-500 ${windowCls} animate-fade-in`}>
      
      {/* Header */}
      <div className={`p-4 text-white font-black flex items-center justify-between flex-shrink-0 ${headerCls}`}>
      ...
      {/* Tabs Menu */}
      <div className={`flex border-b text-xs font-extrabold flex-shrink-0 ${isNight ? 'border-white/[0.08] bg-slate-950' : 'border-gray-200 bg-gray-100'}`}>
        <button 
      ...
      <div className="flex-1 p-4 flex flex-col justify-between overflow-hidden">
        {loading ? (
          <div className="flex justify-center items-center h-full">
            <div className={`flex space-x-1.5 p-3 rounded-xl shadow-sm ${isNight ? 'bg-slate-800' : 'bg-white'}`}>
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            {activeTab === 'today' && (
              <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar flex flex-col">
                {data && data.trim() !== "" ? (
                  <div className={`p-4 rounded-2xl border mb-auto transition-all ${subCardCls}`}>
                  ...
                  <div className="text-center px-4 py-8 flex flex-col items-center justify-center space-y-3 mt-12 my-auto">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center text-xl ${isNight ? 'bg-slate-800' : 'bg-blue-50'}`}>✨</div>
                    <h4 className={`text-[14px] font-black ${isNight ? 'text-gray-300' : 'text-gray-600'}`}>Chưa có đánh giá nào!</h4>
                    <p className={`text-[12px] font-bold text-center max-w-[85%] leading-relaxed ${isNight ? 'text-gray-500' : 'text-gray-400'}`}>Bé hãy bắt đầu bài học để nhận đánh giá vào hôm sau nhé.</p>
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === 'history' && (
              <div className="flex-1 flex flex-col overflow-hidden">
                ...
                {(!detailAssessment && !detailError && !isDetailLoading) && (
                  <div className={`mb-3 p-3 rounded-2xl border flex flex-col gap-1.5 flex-shrink-0 ${subCardCls}`}>
                    <label className={`text-[10px] font-extrabold uppercase tracking-wider px-0.5 ${isNight ? 'text-slate-400' : 'text-gray-400'}`}>🔍 Tra cứu nhanh theo ngày</label>
                ...
                {error ? (
                  ...
                  <div className="flex-1 flex flex-col overflow-hidden justify-between animate-fade-in">
                    <div className="flex flex-col flex-shrink-0">
                      <button 
                        onClick={() => { onResetDetail(); setCustomDate(''); }} 
                      ...
                      <div className={`text-[10px] font-black px-2 py-0.5 rounded-md mb-2 w-fit ${isNight ? 'bg-indigo-950 text-indigo-300 border border-indigo-500/30' : 'bg-indigo-50 text-indigo-700'}`}>
                    ...
                    {isDetailLoading ? (
                      ...
                      <div className={`p-5 rounded-2xl text-red-400 text-xs text-center font-bold my-auto border ${subCardCls}`}>
                        ⚠️ Không tìm thấy đánh giá chi tiết cho ngày này.
                      </div>
                    ) : (
                      <div className={`p-4 rounded-2xl border overflow-y-auto custom-scrollbar flex-1 ${subCardCls}`}>
                        <FormatMarkdown text={detailAssessment} isNight={isNight} />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex-1 flex flex-col overflow-hidden">
                    <p className={`text-[10px] font-extrabold px-0.5 mb-1.5 flex-shrink-0 ${isNight ? 'text-slate-400' : 'text-gray-400'}`}>Chọn từ danh sách dưới đây:</p>
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center gap-2 overflow-hidden">
                                ...
                                <span className={`text-[11px] font-semibold truncate ${isNight ? 'text-slate-400' : 'text-gray-400'}`}>— {dateDisplay}</span>
                              </div>
                              <span className={`text-[11px] font-extrabold flex-shrink-0 ${isNight ? 'text-blue-400' : 'text-blue-500'}`}>Xem →</span>
                            </div>
                          );
                        })
                      ) : (
        ...
        {activeTab === 'history' && !detailAssessment && !detailError && !isDetailLoading && !loading && !error && totalPages > 1 && (
          <div className={`flex justify-between items-center border-t pt-3 text-xs flex-shrink-0 ${isNight ? 'border-white/[0.08]' : 'border-gray-200'}`}>
            <button 
            ...
            <span className={`font-bold text-[11px] ${isNight ? 'text-slate-400' : 'text-gray-500'}`}>Trang {currentPage} / {totalPages}</span>
            <button 
