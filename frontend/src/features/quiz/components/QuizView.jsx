    ...
    <div className="flex h-full flex-col gap-5 relative">
      ...
      {showCelebration && (
        <div className="fixed inset-0 z-[200] flex flex-col items-center justify-center bg-black/75 backdrop-blur-md animate-in fade-in duration-300">
          ...
          <div className={`relative rounded-[2.5rem] p-8 md:p-10 text-center max-w-md mx-4 border-4 shadow-[0_0_50px_rgba(234,179,8,0.2)] animate-in zoom-in duration-500 ${
            isNight 
              ? "bg-[#111a38] border-yellow-500/70 shadow-yellow-500/5" 
              : "bg-white border-yellow-400"
          }`}>
            {/* ĐÃ SỬA: Nền icon thay đổi theo theme */}
            <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-5 animate-bounce ${
            ...
            <h3 className={`text-2xl md:text-3xl font-black uppercase tracking-tight bg-clip-text text-transparent bg-gradient-to-r ${
              isNight 
            ...
            <p className={`text-sm md:text-base font-bold mt-3 leading-relaxed ${
              isNight ? "text-slate-300" : "text-gray-600"
            }`}>
              Chúc mừng bé đã hoàn thành xuất sắc thử thách! Hãy cùng xem lại điểm số và các câu trả lời nhé. ✨
            </p>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_260px] items-start">
        ...
        <div className="space-y-3">
          ...
          {isFilterActive && filteredIndexSet.size === 0 ? (
            <div className={`flex flex-col items-center justify-center rounded-[2rem] border-2 p-12 text-center shadow-sm ${
              isNight ? "bg-[#1a254f]/80 border-blue-500/20" : "bg-gray-50 border-gray-200"
            }`}>
              <span className="text-6xl mb-4 drop-shadow-md">
                {resultFilter === 'wrong' ? '🎉' : '🔍'}
              </span>
              <h3 className={`text-xl font-black uppercase tracking-wide ${isNight ? "text-slate-200" : "text-gray-700"}`}>
                Không có câu hỏi nào
              </h3>
              <p className={`text-sm mt-3 font-semibold ${isNight ? "text-slate-400" : "text-gray-500"}`}>
                {resultFilter === "correct" && "Bé chưa làm đúng câu nào cả. Lần sau cố lên nhé!"}
                {resultFilter === "wrong" && "Tuyệt vời! Bé không làm sai câu nào!"}
                {resultFilter === "unanswered" && "Bé đã hoàn thành tất cả các câu hỏi. Rất chăm chỉ!"}
              </p>
            </div>
          ) : (
            ...
            <>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
                <div className="flex items-center gap-2">
                  <button onClick={handlePrevQuestion} disabled={!canGoPrev} className={`flex items-center gap-1 rounded-xl border px-4 py-2 text-sm font-semibold disabled:opacity-50 transition-all active:scale-95 ${isNight ? "border-[#7d95e2]/45 bg-[#1a254f] text-slate-100 hover:bg-[#253468]" : "border-gray-200 bg-white text-gray-600 hover:bg-gray-50"}`}><ChevronLeft size={18} /> Trước</button>
                  <button onClick={handleNextQuestion} disabled={!canGoNext} className={`flex items-center gap-1 rounded-xl border px-4 py-2 text-sm font-semibold disabled:opacity-50 transition-all active:scale-95 ${isNight ? "border-[#7d95e2]/45 bg-[#1a254f] text-slate-100 hover:bg-[#253468]" : "border-gray-200 bg-white text-gray-600 hover:bg-gray-50"}`}>Tiếp <ChevronRight size={18} /></button>
                </div>
